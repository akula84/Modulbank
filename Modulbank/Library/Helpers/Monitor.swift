...
public class Monitor {
    ...
    public func start() -> Bool {
        ...
        if SCNetworkReachabilitySetCallback(reachabilityReference, Monitor.systemReachabilityCallback(), &networkReachabilityContext) {
            if SCNetworkReachabilityScheduleWithRunLoop(reachabilityReference, CFRunLoopGetCurrent(), RunLoop.Mode.default.rawValue as CFString) {
                return true
            }
        }
    ...
    public func stop() {
        ...
        if reachabilityReference != nil {
            SCNetworkReachabilityUnscheduleFromRunLoop(reachabilityReference, CFRunLoopGetCurrent(), RunLoop.Mode.default.rawValue as CFString)
        }
        unsafeSelfPointer.deallocate()
        //unsafeSelfPointer.deallocate(capacity: 1)
